...
// Custom Glints CHROMATIC MODEL fragment shader (1.21.1). core/chromatic_cutout.fsh without the armor-shape
// alpha-test: the model IS its own shape (elytra), and the EQUAL depth test against the base's cutout depth
// already clips the slick to the base's opaque texels. See chromatic_model.vsh for why this variant carries no
// camera-ward bias. Everything below the removed mask is core/chromatic.fsh line-for-line.
