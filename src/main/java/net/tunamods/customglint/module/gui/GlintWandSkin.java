...
/**
 * A Glint Wand (editor) skin, the wand counterpart to {@link GlintTableSkin}. A background PNG plus a
 * palette for everything the screen draws over it (buttons, swatches, labels, the channel fields). The
 * window frame, column divider and preview recess live in the PNG; the palette colours the rest so the
 * same draw code reads on a light or dark theme. Add a skin by appending it to {@link #ALL} with its
 * palette and a matching {@code glint_wand_<name>.png}; the in-window button cycles them and the choice
 * persists separately from the table's skin.
 */
class GlintWandSkin {

    final String name;

    // Frame / bevel palette (shared shape with GlintTableSkin).
    ...
    /** Blit the background PNG over the full panel. */
    ...
    static {
        ...
        for (GlintWandSkin s : ALL) {
            s.texture(CustomGlint.res("textures/gui/glint_wand_" + s.name.toLowerCase(Locale.ROOT) + ".png"));
        }
    }
